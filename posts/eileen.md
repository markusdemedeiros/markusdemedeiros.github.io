...
Its popularity can be attributed to a number of features, including but not limited to its developer tooling, extensive mathematical library, and organized and active communuity. 
...
Conversely, a version of Iris built using Mathlib-compatible data structures could open the door to exploring variants of Iris as it continues to grow. 
...
The process of reimplementing Iris presents an opportunity to distill key ideas from its implementation, which can lower the bar for new contributors to the project. 
...
- *As an Iris user* I want to understand the components of Iris (aspirationally) to the degree that I could reimplement them.

- *As a Lean user* I want to understand the mechanization techniques that make large, collaborative verification projects feasible and sustainable. 
...
- Prior Art
  ...
  + Read the Mathlib papers
  + Read the details about typeclass inference and structure extension
  + Read the existing Lean Iris paper (implementation of MoSeL in Lean)
- Planning
  + Take inventory of the existing Iris development
  + Find realistic implementation milestones

My past few months have focused on the reading aspect of this plan, alongside some preliminary mechanization experiments. 
This blog post takes the next step. 

## An Inventory of Iris

There are approximately 200 files in Iris, containing 50,000 lines of Rocq code. 
Not all of these files are relevant to Eileen.
Some of them are library code that can be worked on after the fact, and the proofmode portion of it of it has already [been implemented](https://github.com/leanprover-community/iris-lean).
To develop project milestones, the first thing I need to do read enough the Iris source code to determine which parts are which. 
...
I went through each file in the Iris repository in dependency order. 
The tables below are a deliberately over-simplified summary of their contents--not enough to paint a complete picture of the Iris development, but enough to chart a reasonable course to step through more carefully.

*Lines are left blank or italicized if I am still trying to understand them.*
...
I've tried to keep the list of goals minimal, because I expect it to grow.
